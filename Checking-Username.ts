...
new_users.forEach((newUsersname) => {
  let lowerCase: string = newUsersname.toLocaleLowerCase();

  if (
    current_users.map((c_user) =>
      c_user.toLocaleLowerCase()).includes(lowerCase)
    
  ) {
    console.log(
      `The username ${newUsersname} is not available. Please write a diffrent username`
    );
  } else {
